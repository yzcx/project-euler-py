...
def compute():
    TRIALS = 10 ** 7 # Num of turns to simulate, 10mil for statistical accuracy

    visitcounts = [0] * 40 # Array to count how many times player lands on each square
    ...
    for i in range(TRIALS):
        die0 = random.randint(1, 4) # Dice roll and standard movement
        ...
        if location in (7, 22, 36): # Chance squares
    ...
    ans = "".join(f"{i:02}" for (i, c) in temp[: 3]) # Concatenate the indices of top 3 squares
    return str(ans)
